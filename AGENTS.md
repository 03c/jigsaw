...
1. `npm run dev` -- ensures backing services and schema, then HMR (or use `npm run dev:app` if containers are already up and you only need the Vite server)
2. Make changes; the browser reloads automatically
3. `npm run dev:services:down` -- stop backing services when done
...
- **Default dev credentials**: Keycloak admin user is `admin` / `admin` (override via optional `.env.local`).
- **Dev realm generation**: `prepare-dev-realm.mjs` runs before local compose; it loads optional `.env.local` and generates `keycloak/jigsaw-realm.dev.json` from `keycloak/jigsaw-realm.json`. This generated file is gitignored.
...
Local dev: optional `.env.local` (see `.env.local.example`); not required for the default loop

`npm run dev` runs `scripts/dev-preflight.mjs`, which loads `.env.local` with `process.loadEnvFile()` when the file exists (same variables as `prepare-dev-realm.mjs`). Docker Compose for local services uses `scripts/dev-compose.mjs`, which passes `--env-file .env.local` to Compose when present so stack overrides apply. The production Dockerfile does not embed env vars -- they're injected via Docker Compose environment blocks.
