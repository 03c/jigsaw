...
const SITE_WORDPRESS_IMAGE_TEMPLATE =
  process.env.SITE_WORDPRESS_IMAGE_TEMPLATE || "jigsaw-wordpress:{phpVersion}";
...
export function resolveWordPressImage(phpVersion: string): string {
  if (SITE_WORDPRESS_IMAGE_TEMPLATE.includes("{phpVersion}")) {
    return SITE_WORDPRESS_IMAGE_TEMPLATE.replaceAll("{phpVersion}", phpVersion);
  }

  return `${SITE_WORDPRESS_IMAGE_TEMPLATE}:${phpVersion}`;
}
