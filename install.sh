...
KEYCLOAK_CONSOLE_URL=https://auth.${PANEL_DOMAIN}
TRAEFIK_DASHBOARD_URL=https://traefik.${PANEL_DOMAIN}/dashboard/
...
echo -e "    2. You'll be redirected to Keycloak to log in or register"
echo -e "    3. Admin: username ${BOLD}admin${NC}  password: ${BOLD}<the Keycloak admin password you entered>${NC}"
echo -e "    4. Or use ${BOLD}Register${NC} on the Keycloak page to create a user account (no admin access)"
echo -e "    5. Optional: disable public sign-up in Keycloak (Realm settings → Login → User registration)"
echo ""
echo -e "  ${BOLD}Host a WordPress site:${NC}"
echo -e "    Point an A record for your site domain to this server, then in the panel:"
echo -e "    Dashboard → Create site → enable database + Install WordPress → open your domain in a browser"
echo ""
